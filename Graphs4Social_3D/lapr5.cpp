...
int colisaoLivre()
{
	...
	if (n > 0)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			/*
			if(hits > 0){
			estado.camera.center[0] -= k * estado.camera.velH * cos(estado.camera.dir_long);
			estado.camera.center[1] -= k * estado.camera.velH * sin(estado.camera.dir_long);
			estado.camera.center[2] -= k * estado.camera.velTotal;
			}
			*/
