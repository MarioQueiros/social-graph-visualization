...
grauMedio(R):-findall(X,user(X),LU),grauMedio(LU,V,C),R is V/C.
%passos, contagem caminhos		%Passos, Numero caminhos
grauMedio([U|UR],V,C):-somaCaminhos(U,P,NC),grauMedio(UR,VA,RA),V is VA + P, C is RA + NC.
grauMedio([],0).

%passos, n caminhos
somaCaminho(U,P,NC):-findall(X,camCurto(U,_,X),LC),length(LC,NC),contaPassos(LC,P).
