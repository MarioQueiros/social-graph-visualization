...
void createTextures(GLuint texID[])
{
	...
	if(read_JPEG_file(NOME_TEXTURA_TOOLTIP, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_TOOLTIP]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	}else{
		printf("Textura %s not Found\n",NOME_TEXTURA_TOOLTIP);
		exit(0);
	}
	if(read_JPEG_file(NOME_TEXTURA_SMILEY, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_SMILEY]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	if(read_JPEG_file(NOME_TEXTURA_SAD, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_SAD]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	if(read_JPEG_file(NOME_TEXTURA_SLEPPY, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_SLEEPY]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	if(read_JPEG_file(NOME_TEXTURA_PLEASED, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_PLEASED]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	}else{
		printf("Textura %s not Found\n",NOME_TEXTURA_PLEASED);
		exit(0);
	}
	if(read_JPEG_file(NOME_TEXTURA_TEARDS, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_TEARDS]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	if(read_JPEG_file(NOME_TEXTURA_BORED, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_BORED]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	if(read_JPEG_file(NOME_TEXTURA_UPSET, &image, &w, &h, &bpp))
	{
		glBindTexture(GL_TEXTURE_2D, texID[ID_TEXTURA_UPSET]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST );
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
...
void myInit()
{
	...
	mdlviewer_init("mdl/urban.mdl", modelo.urban);
	mdlviewer_init("mdl/scientist.mdl", modelo.scientist);
	mdlviewer_init("mdl/bugsbunny.mdl", modelo.bugsbunny);
	mdlviewer_init("mdl/hostage.mdl", modelo.hostage);
	mdlviewer_init("mdl/nuku_Girl.mdl", modelo.nuku_Girl);
	mdlviewer_init("mdl/Spiderman.mdl", modelo.Spiderman);
	mdlviewer_init("mdl/anonymous.mdl", modelo.anonymous);
	mdlviewer_init("mdl/homer.mdl", modelo.homer);
}
...
void InitAudio()
{
	audio.buffer = alutCreateBufferFromFile("audio/audio.wav");
...
int escreveTexto(void *fonte,string texto,int posX,int posY){
	estado.linhas=0;
	int x=-380, y=-40;
	glColor3ub(255,255,255);	
	int tamanhoLinha=28,contador=0,cont=1;
	string subTexto=texto;
	int flag=0;
	glRasterPos2i(posX,posY);
	for(size_t i = 0; i < texto.size(); ++i)
	{	
		if(subTexto.size()>tamanhoLinha){
			if(i==tamanhoLinha){
				posY=posY-28;
				glRasterPos2i(posX,posY);
				texto="";
				texto = subTexto.substr (28*cont,subTexto.size());
				i=0;
				cont++;
				estado.linhas++;
			}
			...
			if (contador==subTexto.size() || estado.linhas==9){
				break;
			}
		}else if (texto.size()==tamanhoLinha || texto.size()<tamanhoLinha){
			glutBitmapCharacter(fonte, texto[i]);
		}
	}
...
void RenderString(void *fonte)
{ 
	string sexo="",nick="",tags="";
	for(int i=0;i<nos[estado.posMouse.numeroNo-1].profile.size();i++){
		if(nos[estado.posMouse.numeroNo-1].profile[i].find("Sexo:") == 0){
			sexo = nos[estado.posMouse.numeroNo-1].profile[i];
			break;
		}
	}
	...
	int posY=-40,posX=-380;
	if(!nos[estado.posMouse.numeroNo-1].username.compare("")==0){
		texto = "Username: ";
		posY = escreveTexto(fonte,texto,posX,posY);


		texto = nos[estado.posMouse.numeroNo-1].username;
		posY = escreveTexto(fonte,texto,posX+100,posY);
	}

	if(!sexo.compare("")==0){
		texto = "Sexo: ";
		posY = escreveTexto(fonte,texto,posX,posY-28);

		texto = sexo.substr(5,sexo.size());
		posY = escreveTexto(fonte,texto,posX+60,posY);
	}
	if(nos[estado.posMouse.numeroNo-1].tags.size()>0)
		tags = nos[estado.posMouse.numeroNo-1].tags[0];
	for(int i=1;i<nos[estado.posMouse.numeroNo-1].tags.size();i++){
		tags += ","+nos[estado.posMouse.numeroNo-1].tags[i];
	}
...
void desenhatooltip (int x, int y, GLuint texID)
{
	if(estado.posMouse.flag==1){
		...
		RenderString(GLUT_BITMAP_HELVETICA_10);
...
void desenhaNo(int no)
{
	...
	material(white);
	string aux = "";
	for(int i=0;i<nos[no].profile.size();i++){
		if(nos[no].profile[i].find("Imagem:") == 0){
			aux = nos[no].profile[i];
			break;
		}
	}

	if(aux.compare("Imagem:anonymous.png") == 0){
		mdlviewer_display(modelo.anonymous);
	}
	else if(aux.compare("Imagem:bugsbunny.png") == 0){
		mdlviewer_display(modelo.bugsbunny);
	}
	else if(aux.compare("Imagem:Spiderman.png") == 0){
		mdlviewer_display(modelo.Spiderman);
	}
	else if(aux.compare("Imagem:urban.png") == 0){
		mdlviewer_display(modelo.urban);
	}
	else if(aux.compare("Imagem:hostage.png") == 0){
		mdlviewer_display(modelo.hostage);
	}
	else if(aux.compare("Imagem:nuku_Girl.png") == 0){
		mdlviewer_display(modelo.nuku_Girl);
	}
	else if(aux.compare("Imagem:scientist.png") == 0){
	...
	string estado="", texto="";
	for(int i=0;i<nos[no].profile.size();i++){
		if(nos[no].profile[i].find("EstadoHumor:") == 0){
			estado = nos[no].profile[i];
			break;
		}
	}

	if(!estado.compare("")==0){
		texto = estado.substr(12,estado.size());
	}

	if(texto.compare("Bored.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_BORED]);
	}
	else if(texto.compare("Pleased.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_PLEASED]);
	}
	else if(texto.compare("Sad.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_SAD]);
	}
	else if(texto.compare("Sleepy.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_SLEEPY]);
	}
	else if(texto.compare("Smiley.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_SMILEY]);
	}
	else if(texto.compare("Teards") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_TEARDS]);
	}
	else if(texto.compare("Upset.png") == 0){
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_UPSET]);
	}
	else{
		glBindTexture(GL_TEXTURE_2D, modelo.texID[ID_TEXTURA_SMILEY]);
	}
...
void display(void)
{
	...
	desenhatooltip(estado.posMouse.posMouseX,glutGet(GLUT_WINDOW_HEIGHT)-estado.posMouse.posMouseY,modelo.texID[ID_TEXTURA_TOOLTIP]);
	...
	if(estado.teclas.up){
		...
		if(!estado.vooRasante)
		{
			...
			if(rasante > 0)
			{
				...
				estado.camera.eye.x = nx;
				estado.camera.eye.y = ny; 
				estado.camera.eye.z = rasante + DISTANCIA_SOLO;
			}
			estado.camera.velh = 0;
			estado.camera.velv = 0;
		}
